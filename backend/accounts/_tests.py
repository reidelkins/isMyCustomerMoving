...
def test_customuser_crud():
    company = Company.objects.create(name="Some Company")
    user = CustomUser.objects.create(
        first_name="john",
        last_name="doe",
        company=company,
...
def test_get_users(api_client, authenticated_user, invited_info):
    url = reverse(
        "user-list", kwargs={"company": str(authenticated_user.company.id)}
    )
...
def test_make_extra_user(api_client, authenticated_user, invited_info):
    ...
    assert (
        response.json()[0]["status"] == "pending"
        or response.json()[1]["status"] == "pending"
    )
