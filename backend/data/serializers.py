...
class ClientListSerializer(serializers.ModelSerializer):
    zipCode = serializers.SerializerMethodField()  # this will call get_zipCode method to get the zipCode
    tag = serializers.SerializerMethodField()  # this will call get_tags method to get the tags
...
class HomeListingSerializer(serializers.ModelSerializer):
    zipCode = serializers.StringRelatedField()  # assuming that ZipCode has a __str__ method that returns a meaningful string
    tags = HomeListingTagsSerializer(many=True, read_only=True)

    class Meta:
        model = HomeListing
        fields = ['id', 'zipCode', 'address', 'listed', 'price', 'housingType', 
                  'year_built', 'tags', 'city', 'state', 'bedrooms', 'bathrooms', 
                  'sqft', 'lot_sqft', 'roofing', 'garage_type', 'garage', 
                  'heating', 'cooling', 'exterior', 'pool', 'fireplace']
