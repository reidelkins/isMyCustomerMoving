...
export default function AddSecretModal() {
  ...
  const IntegrateSTSchema = Yup.object().shape({
    clientID: Yup.string("The Client ID is a string that should start with 'cid.'").required(
      'Service Titan Client ID is required'
    ),
    clientSecret: Yup.string('The Client Secret is a long string that is only generated one time').required(
      'Service Titan Client Secret is required'
    ),
  });
        ...
        <Fade in={integrateInfo}>
          ...
          >
            ...
            <Typography id="modal-modal-description" sx={{ mt: 2 }}>
              1. Now that you have submitted your Tenant ID, we will add your ID to our Application and send an email to
              notify you that it has been completed. <br />
              <br />
              2. You will then need to enable the IMCM application in your Service Titan account by going to Settings{' '}
              {'>'} Integrations {'>'} API Application Process. <br />
              <br />
              3. At this point, you will see the Client ID and Client Secret in your Service Titan account. <br />
              <br />
