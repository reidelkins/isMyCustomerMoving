...
test.describe('filter', () => {
  ...
  test('by zip code', async ({ page }) => {
    const filterResponsePromise = page.waitForResponse(`**/api/v1/data/recentlysold/?page=1&tags=&zip_code=${ZIP_CODE}`);
    ...
    await page.getByLabel('This will filter for the city state and zip of the home').getByRole('spinbutton').fill(ZIP_CODE);
    await page.getByLabel('This will filter for the city state and zip of the home').getByRole('spinbutton').press('Enter');
  ...
  test('by city', async ({ page }) => {
    const filterResponsePromise = page.waitForResponse(`**/api/v1/data/recentlysold/?page=1&tags=&city=${CITY}`);
    await page.getByLabel('Filter list').click();
    await page.locator('div').filter({ hasText: /^City$/ }).getByRole('textbox').click();
    await page.locator('div').filter({ hasText: /^City$/ }).getByRole('textbox').fill(CITY);
  ...
  test('by state', async ({ page }) => {
    const filterResponsePromise = page.waitForResponse(`**/api/v1/data/recentlysold/?page=1&tags=&state=${STATE}`);
    await page.getByLabel('Filter list').click();
    await page.locator('div').filter({ hasText: /^State$/ }).getByRole('textbox').click();
    await page.locator('div').filter({ hasText: /^State$/ }).getByRole('textbox').fill(STATE);
    await page.getByRole('button', { name: 'Apply Filters' }).click();
